...
import { refresh, stateful, stateless, withEffect, withProps, withState } from '../../main/index'
...
const
  store = createStore(todoReducer, { todos: load() }),
  dispatch = store.dispatch

const Header = stateful('Header', c => {
  const
    [state, setState] = withState(c, { title: '' }),
...
const Filters = stateless<FilterProps>('Filters', ({ filter }) => {
  return (
    <ul className="filters">
      <li>
        <Link href="#/" className={filter === TodoFilter.None ? 'selected' : ''}>
          All
        </Link>
      </li>
      <li>
        <Link href="#/active" className={filter === TodoFilter.Active ? 'selected' : ''}>
          Active
        </Link>
      </li>
      <li>
        <Link href="#/completed" className={filter === TodoFilter.Completed ? 'selected' : ''}>
          Completed
        </Link>
      </li>
    </ul>
...
const App = stateful('App', c => {
  ...
  return () =>
    <Router history={history}>
      <div path="/active">
        {getFilteredContent(TodoFilter.Active)}
      </div>
      <div path="/completed">
        {getFilteredContent(TodoFilter.Completed)}
      </div>
      <div path="/">
        {getFilteredContent(TodoFilter.None)}
      </div>
    </Router>
})
